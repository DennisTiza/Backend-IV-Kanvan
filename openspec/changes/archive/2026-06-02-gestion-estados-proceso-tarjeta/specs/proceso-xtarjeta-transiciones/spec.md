...
El sistema DEBE exponer `POST /proceso-x-tarjeta/{id}/iniciar` que establece `fechaInicio` a la fecha/hora actual.
...
- **THEN** el sistema DEBE establecer `fechaInicio` a la fecha/hora actual
...
El sistema DEBE exponer `POST /proceso-x-tarjeta/{id}/finalizar` que establece `fechaFinal` a la fecha/hora actual.
